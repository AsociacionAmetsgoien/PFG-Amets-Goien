          ...
          <div className="max-w-7xl mx-auto">
            {loading ? (
              <div className="text-center py-20">
                ...
                <p className="text-2xl text-[#8A4D76] mt-4">{t('activities.loading')}</p>
              </div>
            ) : actividades.length === 0 ? (
              <div className="text-center py-20">
                <p className="text-2xl text-gray-600 font-semibold">{t('activities.noActivities')}</p>
                <p className="text-lg text-gray-500 mt-2">{t('activities.comeSoon')}</p>
              </div>
            ) : (
                    ...
                    <div className="bg-gradient-to-r from-[#8A4D76] to-[#A65D8E] px-5 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-white">
                          {actividad.fecha || t('activities.dateConfirm')}
                        </span>
                      </div>
                      <div className="bg-white/20 px-2 py-1 rounded">
                        <span className="text-xs text-white font-medium">{t('activities.activity')}</span>
                      </div>
                    </div>
        ...
        <section className="py-16 px-4 md:px-8 lg:px-16 bg-white border-t border-gray-200">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl font-bold text-[#8A4D76] mb-4">
              {t('activities.needInfo.title')}
            </h2>
            <p className="text-lg text-gray-700 mb-8 max-w-2xl mx-auto">
              {t('activities.needInfo.subtitle')}
            </p>
            <a href="/contacto">
              <button 
